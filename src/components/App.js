        ...
        >
          {({
            ...
            <form noValidate className="user">
              ...
              <Input className="user__field-row" name="lastName" label="Фамилия" onChange={handleChange}
                     value={values.lastName}
                     error={Boolean(errors.lastName)} errorText={errors.lastName} required/>

              <Input className="user__field-row" name="firstName" label="Имя" onChange={handleChange}
                     value={values.firstName}
                     error={Boolean(errors.name)} errorText={errors.name} required/>

              <Input className="user__field-row" name="patronymic" label="Отчество" onChange={handleChange}
                     value={values.patronymic}/>
              <div className="user__field-row">
                <SelectSex className="user__field-cell user__field-cell--left" options={options} onChange={handleChange}
                           value={values.sex}/>

                <div className="user__field-cell user__calendar">
                  <Datepicker className="user__field-cell user__calendar"
              ...
              <div className="user__field-row">
                <Phone className="user__field-cell user__field-cell--left" onChange={e => {
                  const value = e.target.value || '';
                  setFieldValue('phone', value);
                }} value={values.phone}
                       errorText={errors.phone}
                       required/>
                <Input className="user__field-cell"
                       name="email"
              ...
              <Input className="user__field-row" name="address" label="Адрес постоянной регистрации"
                     onChange={handleChange}/>
              <Input className="user__field-row" name="company" label="Название работадателя" onChange={handleChange}/>
              <div className="user__field-row">
                <div className="user__field-cell"/>
                <Button className="user__field-cell" onClick={() => {
                  submitForm()
                }} disabled={isSubmitting}>{btnTitle}</Button>
              </div>
            </form>
          )}
        </Formik>
